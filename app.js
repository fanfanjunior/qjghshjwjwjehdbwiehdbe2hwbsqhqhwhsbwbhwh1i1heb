...
try {
  ...
  db.query('SELECT * FROM users', (err, result) => {
    if (err) {
      console.error('Error querying users for auto delete:', err);
      return;
    }
  ...
  if (daysDifference >= 7 && user.otp_verified === 0) {
    db.query('DELETE FROM users WHERE id = ?', [user.id], (err) => {
      if (err) {
        console.error('Error deleting unverified user:', err);
      } else {
        console.log(`User ${user.username} deleted due to unverified account for 7 days.`);
      }
    });
  }
});
      ...
      db.query(
        ...
        (err) => {
          if (err) {
            ...
            console.log(`User ${user.username} has been reset to Free with a usage limit of ${usage_limit_amount.free}.`);
          }
        }
      );
    });
