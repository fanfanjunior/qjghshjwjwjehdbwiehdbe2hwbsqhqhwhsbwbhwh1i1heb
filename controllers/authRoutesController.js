...
exports.post_signup = (req, res) => {
  ...
  //const verifycationExpirationDate = new Date(currentDate.getTime() + 7 * 24 * 60 * 60 * 1000);
      ...
      if (result.length > 0) {
        ...
        sendEmailOTP(email, otp, (err) => {
          if (err) {
            ...
            db.query('INSERT INTO users (email, username, password, api_key, usage_limit, otp, registration_date) VALUES (?, ?, ?, ?, ?, ?, ?)', [email, username, password, apiKey, usage_limit_amount.free, otp, currentDate], (err) => {
            if (err) {
              console.error(err);
              return res.status(500).send('Terjadi kesalahan pada server.');
            }
